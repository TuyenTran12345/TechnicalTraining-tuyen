...
class ReportL10nVnS07dn(models.AbstractModel):
    _name = 'report.l10n_vn_viin_account_reports.report_s07dn'
    _inherit = 'report.l10n_vn.common'
    _description = 'VietNam S07dn Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        res = super(ReportL10nVnS07dn, self)._get_report_values(docids, data)
    ...
    def group_by_account_id(self):
        ...
        initial_bal_date_to = fields.Date.to_date(ctx_date_from) + timedelta(days=-1)
        initial_bal_results = self.with_context(
            date_from=ctx_date_from and company.compute_fiscalyear_dates(fields.Date.to_date(ctx_date_from))[
                'date_from'] or None,
            strict_range=False,
    ...
    def _do_query(self, line_id, group_by_account=True):
        results = []
        if self.env['account.move.line'].check_access_rights('read'):
    ...
    def do_query(self, line_id=None):
        ...
        current_company = self.env['res.company'].browse(self._context.get('company_id', [])).exists() or self.env.company
    ...
    def _get_ctp_amls(self, ctx, line):
        """
        @param ctx: {'date_to': '2023-01-01', 'date_from': '2023-12-31', ...}
        @param line: account.move.line(1,)
        @return account.move.line(2,), account.move.line(3,)
        E.g:
        ...
        """
        direct_ctp_amls = self.env['account.move.line']
        indirect_ctp_amls = self.env['account.move.line']
        # Get counterpart journal item of line parameter
        if line:
            direct_ctp_amls = line.dr_ctp_ids.dr_aml_id if line.balance < 0 else line.cr_ctp_ids.cr_aml_id
        # Get counterpart journal item of indirect_ctp_amls
        date_to = fields.Date.to_date(ctx['date_to'])
        for aml in direct_ctp_amls:
            if aml.balance < 0:
                # Only get matched reconcile before date in date_to context parameter
                indirect_ctp_amls |= aml.matched_debit_ids.filtered(lambda m: m.max_date <= date_to).debit_move_id
            else:
                indirect_ctp_amls |= aml.matched_credit_ids.filtered(lambda m: m.max_date <= date_to).credit_move_id
        return direct_ctp_amls, indirect_ctp_amls
    ...
    def _get_lines(self, data):
        ...
        for account in sorted_accounts:
            ...
            progress_credit = initial_credit
            for aml in amls:
                line = aml.get('line', self.env['account.move.line'])
                if line.is_debit():
                    progress_debit = progress_debit + line.debit
                else:
                    progress_credit = progress_credit + line.credit
                name = aml.get('sum_name', '')
                if line.ref and line.ref != line.name:
                    if name:
                        name += '<br/>%s' % line.ref
                    else:
                        name = line.ref
                direct_ctp_amls, indirect_ctp_amls = self._get_ctp_amls(ctx, line)
                report_lines.append({
                    'type': 'move_line',
                    'name': name,
                    'direct_ctp_account_id': ', '.join(direct_ctp_amls.account_id.mapped('code')),
