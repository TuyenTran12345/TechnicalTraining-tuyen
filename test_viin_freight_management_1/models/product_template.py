...
class ProductTemplate(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            if rec.port_id:
                port_info = f'{rec.port_id.ref} ({rec.port_id.country_id.code})'
                if port_info in rec.name:
                    result.append((rec.id, rec.name))
                else:
                    result.append((rec.id, '%s - %s (%s)' % (rec.name, rec.port_id.ref, rec.port_id.country_id.code)))
            else:
                result.append((rec.id, rec.name or ''))
        return result
