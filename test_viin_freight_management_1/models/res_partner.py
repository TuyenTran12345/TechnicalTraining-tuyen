...
class ResPartner(models.Model):
    ...
    # Todo: Move me to to_geo_routes module when upgrade version 17.0
    ...
    def name_get(self):
        res = super().name_get()
        result = []
        for partner, res_item in zip(self, res):
            if partner.is_port:
                country_name = partner.country_id.name.upper() if partner.country_id and partner.country_id.name else ''
                country_code = partner.country_id.code.upper() if partner.country_id and partner.country_id.code else ''
                name = "%s - %s (%s)" % (country_name, partner.ref or '', country_code)
            else:
                name = res_item[1]
