...
class FreightTransportMixin(models.AbstractModel):
    ...
    total_packages = fields.Integer(
        ...
        compute='_compute_counts', compute_sudo=True, store=True,
        help="Total number of packages in the shipment"
    )
    total_cargo_commodities = fields.Integer(
        string='Total Cargo Commodities',
        compute='_compute_counts', readonly=False, compute_sudo=True, store=True,
    ...
    def _onchange_service_type(self):
        ...
        if not self.route_id:
            return
        start_point = self.route_id.address_from_id
        end_point = self.route_id.address_to_id
