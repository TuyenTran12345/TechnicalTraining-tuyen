...
class TestFreightRouteFeatures(TestCommon):
    ...
    def test_002_display_name_method(self):
        """Test Case: Test display_name method

        Display name must return correct format with route type.
        """
        so = self.sale_order_sea_fcl_40ft
        so._action_confirm()
        shipment = so.shipment_ids[:1]
        ...
        name_get_result = route.display_name

        self.assertIn('MAIN', name_get_result, "Display name must contain uppercase route type")
        self.assertIn(route.origin_id.name, name_get_result, "Display name must contain origin name")
        self.assertIn(route.destination_id.name, name_get_result, "Display name must contain destination name")
